...
class DataLoader():
    ...
    def save_to_db(self, splitted_docs, embeddings):
        ...
        # Collect the texts that will be embedded
        texts = [doc.page_content for doc in splitted_docs]
        # print(f"Preparing to embed {len(texts)} chunks")

        # Log the type of the first few items to detect structure issues
        # for i, t in enumerate(texts[:3]):
        #     print(f"[Chunk {i}] Type: {type(t)}, Value (first 200 chars): {repr(t[:200])}")
        ...
        try:
            ...
            print("Error during Chroma.from_documents() call:")
            raise
